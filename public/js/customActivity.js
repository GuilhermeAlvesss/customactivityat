...
define(function (require) {
    ...
    function onRender() {
        ...
        $('#toggleActive').click(function (evt) {
            ...
            if (validate()) {
                ...
                document.getElementById('toggleActive').style.background = "grey";
                document.getElementById('toggleActive').innerHTML = "Pronto";
            }
        });
    }
    ...
    function initialize(data) {
        ...
        if (templateCode) {
            ...
            document.getElementById('toggleActive').disabled = true;
            document.getElementById('toggleActive').innerHTML = "Ativar";
        }
    }

    function onGetTokens(tokens) {
        console.log('LINHA 124 => Token: ' + tokens);
        authTokens = tokens;
    }

    function onGetEndpoints(endpoints) {
        console.log('LINHA 129 => Endpoints: ' + endpoints);
    }

    // função usada para capturar o id da entrada de dados da jornada, usada na linha 21
    function requestedInteractionHandler(settings) {
        try {
            eventDefinitionKey = settings.triggers[0].metaData.eventDefinitionKey;
            document.getElementById('select-entryevent-defkey').value = eventDefinitionKey;
            console.log('LINHA 137 => Event Definition Key: ' + eventDefinitionKey);
        } catch (err) {
            console.error(err);
        }
    }

    // função chamada no connection.on na linha 22
    function save() {
        ...
        console.log('LINHA 159 => Payload Completo: ', JSON.stringify(payload)); // manda pro console do navegador o JSON que foi gerado pelo payload

        connection.trigger('updateActivity', payload); // fecha o modal quando clicar no botao DONE
